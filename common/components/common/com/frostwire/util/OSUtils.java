...
public class OSUtils {
    ...
    public static boolean isModernWindows() {
        return isWindows() && !(_isWindows98 || _isWindows95 || _isWindowsMe || _isWindowsNT || _isWindowsXP);
    }
    ...
    private static void detectAndroidOS() {
        ...
        try {
            Class<?> androidBuild = Thread.currentThread().getContextClassLoader().loadClass("android.os.Build");
            String manufacturer = (String) androidBuild.getField("MANUFACTURER").get(null);
            String model = (String) androidBuild.getField("MODEL").get(null);

            if (manufacturer.equals("Amazon") && (model.equals("Kindle Fire") || model.startsWith("KF"))) {
                isKindleFire = true;
            }
        } catch (Throwable e) {
            // ignore
