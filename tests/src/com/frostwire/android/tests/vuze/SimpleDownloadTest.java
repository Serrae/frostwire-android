...
public final class SimpleDownloadTest extends AbstractTorrentTest {
    ...
    public void testDownload1() throws Exception {
        ...
        File f = new File(saveDir, FilenameUtils.getName(url));
        TOTorrent t = TestUtils.downloadTorrent(url);

        t.serialiseToBEncodedFile(f);

        VuzeManager.getInstance().getEngine().remove(t.getHash(), true, true);
        ...
        assertTrue("Download didn't finish on time", TestUtils.await(downloadFinished, 5, TimeUnit.MINUTES));
    }
}